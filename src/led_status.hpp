...
public:
    ...
    void update() {
        ...
        if (currentTime - sequenceStartTime >= (statusDuration + separatorDuration) * activeStatuses.size()) {
        ...
        if (elapsedInCycle < statusDuration) {
            ...
            for (const auto& statusPair : statusColorMap) {
                if (statusPair.first == currentStatus) {
                    setColor(statusPair.second.r, statusPair.second.g, statusPair.second.b, statusPair.second.blink);
                    break;
                }
            }
        } else {
...
private:
    void setupStatusColorMap() {
        ...
        statusColorMap.push_back({LED_STATUS_ROS_CONNECTED, {0, 255, 0, false}}); // Green solid for ROS connected
        statusColorMap.push_back({LED_STATUS_CHARGING, {255, 255, 0, true}});     // Yellow blinking for charging
        statusColorMap.push_back({LED_STATUS_DISCHARGING, {255, 0, 255, true}});    // Magenta blinking for discharging
        statusColorMap.push_back({LED_STATUS_BATTERY_LOW, {255, 0, 0, false}});   // Red for battery low
    }
